...
export async function GET(req: Request) {
    ...
    // Fetch every order that's still pending payment with a Paystack reference
    const { data: pendingOrders, error } = await supabaseAdmin
        .from("orders")
        .select("id, customer_email, customer_name, customer_phone, total_amount, items, customer_metadata, paystack_reference, discount_code, discount_amount, delivery_method")
    ...
    await Promise.allSettled(orders.map(async (order) => {
        try {
            ...
            if (paystackStatus === "success") {
                ...
                // Mark paid — scoped to payment_status='pending' for idempotency
                await supabaseAdmin
                    .from("orders")
                    .update({
                        status: "paid",
                ...
                await supabaseAdmin
                    .from("orders")
                    .update({ status: "failed", payment_status: "abandoned" })
                    .eq("id", order.id)
                    .eq("payment_status", "pending");
